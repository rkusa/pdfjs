...
import sRgbProfile from './sRGB_IEC61966-2-1_black_scaled'
...
export default class Document extends Readable {
  constructor(opts) {
    if (!opts) {
      opts = {}
    }
    ...
    this.defaultColor = opts.color && colorToRgb(opts.color) || [0, 0, 0]
    ...
    this.paddingTop = defaults(opts.paddingTop, padding)
    this.paddingBottom = defaults(opts.paddingBottom, padding)
    this.paddingLeft = defaults(opts.paddingLeft, padding)
    this.paddingRight = defaults(opts.paddingRight, padding)
    ...
    // init pages catalog
    this._pages = new PDFArray()
    this._pagesObj = new PDFObject('Pages')
    this._registerObject(this._pagesObj)

    // init destinations name tree
    this._destinations = new PDFNameTree()

    // init outlines hierarchy
    this._outlines = []

    // init color space
    this._colorSpace = new PDFObject()
    const iccProfile = sRgbProfile + '~>'
    ...
    // this._colorSpace.prop('Filter', new PDFArray([
    //   new PDFName('ASCII85Decode'), new PDFName('FlateDecode')
    // ]))
    this._colorSpace.prop('Filter', new PDFName('ASCII85Decode'))
  ...
  _useFont(font) {
    ...
    if (!alias) {
      alias = new PDFName(this._aliases.next('F'))
  ...
  _useXObject(xobj) {
    ...
    if (!alias) {
      alias = new PDFName(this._aliases.next('X'))
      this._mapping.set(xobj, alias)

      const xobjObjs = []
      for (let i = 0; i < xobj.xobjCount; ++i) {
        const xobjObj = new PDFObject('XObject')
  ...
  async _startPage() {
    ...
    const page = this._currentPage = new PDFObject('Page')
    ...
    if (this._header) {
      ...
      for (const i in this._header._objects) {
        ...
        const alias = new PDFName(this._aliases.next('H'))
    ...
    if (this._footer) {
      ...
      for (const i in this._footer._objects) {
        ...
        const alias = new PDFName(this._aliases.next('F'))
        this._currentContent._xobjects[alias] = obj.toReference()
        chunk += ops.Do(alias)
      }
  ...
  async _endPage() {
    ...
    const xobjects = new PDFDictionary({})
    ...
    const colorSpace = new PDFDictionary({
      [this._currentColorSpace]: new PDFArray([new PDFName('ICCBased'), this._colorSpace.toReference()])
    })
    page.prop('Resources', new PDFDictionary({
    ...
    if (this._annotations.length > 0) {
      page.prop('Annots', new PDFArray(this._annotations))
      this._annotations = []
    }

    const contents = this._contents.map(c => c._object.toReference())
    page.prop('Contents', new PDFArray(contents))
  ...
  async end() {
    ...
    this._pagesObj.prop('MediaBox', new PDFArray([0, 0, this.width, this.height]))
  ...
  outline(title, destination, parent) {
    ...
    if (this._outlines.length === 0 ){
      this._outlines[0] = new PDFObject('Outlines')
    ...
    // Create item
    const outline = new PDFObject()
    outline.data = {title, destination, parent}
    outline.prop('Title', new PDFString(title))
    outline.prop('Parent', this._outlines[parentIndex].toReference())
    outline.prop('A', new PDFDictionary({
      S: 'GoTo',
      D: new PDFString(destination),
    }))
