...
import ops from './ops'
import {PDFName, PDFArray, PDFDictionary, PDFObject} from './object'
...
export default class Header extends Base {
  ...
  async _setup() {
    ...
    this._resources = new PDFObject()
    this._doc._registerObject(this._resources)
    this._bbox = new PDFObject()
  ...
  _createObject() {
    ...
    const xobj = new PDFObject('XObject')
  ...
  async _render() {
    ...
    for (const instance of this._pageNumbers) {
      ...
      if (withPageCount) {
        ...
        xobj.prop('Subtype', 'Form')
        xobj.prop('FormType', 1)
        xobj.prop('BBox', new PDFArray([instance.x, instance.y, instance.x + instance.width, instance.y - instance.height]))
        xobj.prop('Resources', new PDFDictionary({
          ColorSpace: new PDFDictionary({
            CS1: new PDFArray([new PDFName('ICCBased'), this._doc._colorSpace.toReference()]),
          }),
          ProcSet: new PDFArray([new PDFName('Text')]),
          Font:    fonts,
        }))
        ...
        const alias = new PDFName(this._doc._aliases.next('X'))
  ...
  async _end() {
    ...
    this._resources.content = new PDFDictionary({
      ColorSpace: new PDFDictionary({
        CS1: new PDFArray([new PDFName('ICCBased'), this._doc._colorSpace.toReference()]),
      }),
    ...
    // setup the BBox
    this._bbox.content = new PDFArray([
