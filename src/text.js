...
class LinkRangeStyle extends RangeStyle {
  ...
  _applyStyle(textChunk) {
    this.doc._annotations.push(new PDFDictionary({
      Type: 'Annot',
      Subtype: 'Link',
      Rect: new PDFArray([this.from, this.y, this.from + this.width, this.y + this.height]),
      Border: new PDFArray([0, 0, 0]),
      A: new PDFDictionary({
        Type: 'Action',
        S: 'URI',
        URI: new PDFString(this.link),
      }),
    }))
    return ''
  }
...
class DestinationRangeStyle extends RangeStyle {
  ...
  _applyStyle(textChunk) {
    this.doc._destinations.set(this.destination, new PDFArray([
