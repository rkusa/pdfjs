...
import {rgbEqual} from './util'
import ops from './ops'
import Cell from './cell'

export default class Row {
  constructor(doc, parent, opts) {
    Base.prototype._init.call(this, doc, parent)
  ...
  _begin(ctx) {
    Base.prototype._begin.call(this, ctx)
  }
  ...
  async _drawBorders(isPageBreak, isInsideBreak) {
    ...
    if (this._borderVerticalWidths) {
      for (let i = 0, len = this._borderVerticalWidths.length; i < len; ++i) {
        ...
        if (bw > 0) {
          ...
          if (!borderColor || !rgbEqual(borderColor, bc)) {
    ...
    if (this._hasTopBorder && this._topBorderWidth > 0) {
      ...
      // stroking color
      if (!borderColor || !rgbEqual(borderColor, this._topBorderColor)) {
    ...
    if (!isPageBreak && this._bottomBorderWidth > 0) {
      ...
      // stroking color
      if (!borderColor || !rgbEqual(borderColor, this._bottomBorderColor)) {
