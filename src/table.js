...
import {colorToRgb} from './util'
import ops from './ops'
import {PDFName} from './object'
...
export default class Table extends Base {
  ...
  async _renderHeader(isPageBreak) {
    ...
    for (const obj of this._header._objects) {
      const alias = new PDFName('TH' + obj.id)
  ...
  _row(opts, isHeader) {
    ...
    const NextRow = isHeader ? TableHeader : Row
    const ctx = new NextRow(this._doc, this, opts)
    this._begin(ctx)
    ...
    if (this.borderHorizontalWidths) {
      ...
      ctx._topBorderColor = colorToRgb(this.borderHorizontalColors(this._rowCount))

      if (!isHeader) {
        ctx._bottomBorderWidth = this.borderHorizontalWidths(this._rowCount + 1)
        ctx._bottomBorderColor = colorToRgb(this.borderHorizontalColors(this._rowCount + 1))
      }
...
function applyOpts(opts) {
  ...
  if (opts.borderVerticalColors) {
    ...
    this.borderVerticalColors = opts.borderVerticalColors.map(c => colorToRgb(c))
  }
  // opts.borderVerticalColor
  else if (typeof opts.borderVerticalColor === 'number') {
    this.borderVerticalColors = []
    const color = colorToRgb(opts.borderVerticalColor)
    for (let i = 0; i <= this.widths.length; ++i) {
  ...
  if (!this.borderVerticalColors) {
    this.borderVerticalColors = []
    for (let i = 0; i <= this.widths.length; ++i) {
      this.borderVerticalColors.push(colorToRgb(borderColor))
    }
  }
