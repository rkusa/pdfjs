...
export default class PDFString {
  constructor(str) {
    this.str = str
  }
  ...
  static async parse(xref, lexer, trial) {
    const literal = await PDFString.parseLiteral(lexer, trial)
    const hex = literal === undefined && await PDFString.parseHex(lexer, trial)
  ...
  static async parseLiteral(lexer, trial) {
  ...
  static async parseHex(lexer, trial) {
    if (lexer.getString(1) !== '<') {
      if (trial) {
        return undefined
      }
