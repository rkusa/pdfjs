...
export default class PDFReference {
  constructor(obj) {
  ...
  static async parse(xref, lexer, trial) {
    ...
    return new PDFReference(parseObject.bind(this, xref, lexer.outer, id))
  }
}

async function parseObject(xref, lexer, id) {
  const PDFObject = this.PDFObject
  const Lexer = lexer.constructor
  ...
  if (offset === null) {
    ...
    if (entry.compressed) {
      if (!entry.obj) {
        ...
        const obj = await PDFObject.parse.call(this, xref, lexer)
      ...
      if (!inner.obj) {
        ...
        inner.obj = await PDFObject.parseInner.call(this, xref, innerLexer)
      }
    ...
    lexer.pos = offset
    return await PDFObject.parse.call(this, xref, lexer)
  }
}
