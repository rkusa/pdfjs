...
export default class PDFXref {
  ...
  static async parse(_, lexer, trial) {
    const xref = new PDFXref()

    if (lexer.getString(4) !== 'xref') {
      return await this.parseXrefObject(_, lexer, trial)
    }
  ...
  static async parseXrefObject(_, lexer, trial) {
    const xref = new PDFXref()

    let obj

    try {
      obj = await PDFObject.parse.call(this, xref, lexer, trial)
    } catch (_) {
    ...
    const stream = await inflate(obj)
