...
import { PDFDictionary, PDFArray, PDFString, PDFName } from '../object'

export default async function(img, doc, parent, opts) {
  if (!img) {
  ...
  if (opts.link) {
    doc._annotations.push(new PDFDictionary({
      Type: 'Annot',
      Subtype: 'Link',
      Rect: new PDFArray([x, y, x + renderWidth, y + renderHeight]),
      Border: new PDFArray([0, 0, 0]),
      A: new PDFDictionary({
        Type: 'Action',
        S: 'URI',
        URI: new PDFString(opts.link),
      }),
    }))
  }
  if (opts.goTo) {
    doc._annotations.push(new PDFDictionary({
      ...
      A: new PDFDictionary({
        S: 'GoTo',
        D: new PDFString(opts.goTo),
      }),
    }))
  }
  if (opts.destination) {
    doc._destinations.set(opts.destination, new PDFArray([
