...
import {PDFArray, PDFObject, PDFStream} from '../object'
...
export default class PDFImage {
  ...
  async _initialize() {
    ...
    const parser = new Parser(this._src)
    await parser.parse()
  ...
  async write(doc, xobjs) {
    ...
    const bbox = new PDFArray([0, 0, this.width, this.height])
    ...
    for (let i = 0; i < this.xobjCount; ++i) {
      ...
      xobj.prop('BBox', bbox)
      xobj.prop('Resources', resources instanceof PDFObject ? resources.toReference() : resources)
      ...
      const content = new PDFStream(xobj)
