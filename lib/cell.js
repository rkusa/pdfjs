...
module.exports = class Cell extends Fragment {
  constructor(doc, parent, opts) {
    ...
    if ('padding' in opts) {
      this.paddingTop = this.paddingRight = this.paddingBottom = this.paddingLeft = opts.padding
    }

    // background creation callback
    this.backgroundColor = util.colorToRgb(opts.backgroundColor)

    this.borderTopWidth = opts.borderTopWidth || 0
    this.borderTopColor = util.colorToRgb(opts.borderTopColor || 0x000000)

    this.borderRightWidth = opts.borderRightWidth || 0
    this.borderRightColor = util.colorToRgb(opts.borderRightColor || 0x000000)

    this.borderBottomWidth = opts.borderBottomWidth || 0
    this.borderBottomColor = util.colorToRgb(opts.borderBottomColor || 0x000000)

    this.borderLeftWidth = opts.borderLeftWidth || 0
    this.borderLeftColor = util.colorToRgb(opts.borderLeftColor || 0x000000)
    ...
    this._cursor.bottom  = this._cursor.startY - this._cursor.height + this.paddingBottom
  ...
  async _pageBreak(level) {
    ...
    let contents, offset
    if (this._firstPage && height / this._doc.height <= .15) {
    ...
    this._pending.unshift(async () => {
      ...
      if (level === 1) {
        ...
        this._cursor.bottom += this.paddingBottom
      }
    })
  }
  ...
  async _createBackground() {
    ...
    const bottom = this._cursor.bottom - this.paddingBottom
    ...
    if (this.borderTopWidth > 0) {
      ...
      const x1 = this._cursor.x - this.paddingLeft
    ...
    if (this.borderLeftWidth > 0) {
      ...
      const x = this._cursor.x - this.paddingLeft + this.borderLeftWidth / 2
      const y1 = this._startY - this.borderTopWidth / 2
    ...
    if (this.borderRightWidth > 0) {
      ...
      const x = this._cursor.x - this.paddingLeft + this.outerWidth - this.borderRightWidth / 2
      const y1 = this._startY - this.borderTopWidth / 2
      const y2 = this._startY - height + this.borderBottomWidth / 2
  ...
  async _end() {
    ...
    // restore cursor
    this._cursor.x = this._cursor.startX - this.paddingLeft
  }
}
