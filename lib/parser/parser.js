...
import {PDFXref, PDFTrailer, PDFDictionary, PDFObject, PDFReference} from '../object'
...
class Parser {
  ...
  parse() {
    ...
    this.trailer = this.xref.trailer || PDFTrailer.parse(this.xref, lexer)
    ...
    while (trailer.has('Prev')) {
      ...
      trailer = xref.trailer || PDFTrailer.parse(xref, lexer)
    }
  }
