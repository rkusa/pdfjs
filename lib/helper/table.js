...
function drawRow(row, n, columns) {
  ...
  row.cells.forEach(function(cell, pos) {
    ...
    var corners = [
      [x, self.y - height + cell.options.borderWidth],
      [x + columns[pos] - cell.options.borderWidth, self.y - height + cell.options.borderWidth],
      [x + columns[pos] - cell.options.borderWidth, self.y],
      [x, self.y]
    ]
    corners.push(corners[0])
    
    for (var i = 0; i < 4;)
      drawLine.call(self, corners[i], corners[++i])
...
function drawLine(from, to) {
  ...
  this.page.contents.writeLine('1.00 w')
