...
Table.prototype._draw = function() {
  ...
  if (minWidth && highestRowWidth < minWidth) {
    ...
    for (var i = 0, len = columns.length; i < len; ++i) {
      if (columns[i] > widthPerCell) used += columns[i] - widthPerCell
  ...
  this.rows.forEach(function(row) {
    row.cells.forEach(function(cell, column) {
      ...
      cell.text = new Text(self.page, cell.text, mergeOption(cell.options, {
...
var Row = function(table, options, definition) {
  ...
  // console.log(this.options.borderWidth)
  this.cells = []
    ...
    if (height > this.table.page.document.innerHeight) {
      ...
      var row = new Row(this.table, this.options, function() {
        ...
        self.cells.forEach(function(cell) {
          ...
          newCell.text = new Text(self.table.page, newCell.text, mergeOption(newCell.options, {
