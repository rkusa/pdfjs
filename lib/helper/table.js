...
Table.prototype._draw = function() {
  ...
  var highestRowWidth = Math.max.apply(Math, this.rows.map(function(row) {
    return row.cells.map(function(cell, i) {
      if (self.columns[i] === undefined || cell.width > self.columns[i])
        self.columns[i] = cell.width
      return cell.width
  ...
  this.rows.forEach(function(row) {
    row.cells.forEach(function(cell, column) {
      cell.innerWidth = self.columns[column] - (cell.width - cell.innerWidth)
...
Row.prototype._draw = function(pos) {
  ...
  this.cells.forEach(function(cell, column) {
    ...
    x += self.table.columns[column] - cell.options.borderWidth
  })
...
Cell.prototype._draw = function(x, height, column) {
  ...
  if (this.options.borderWidth > 0) {
    ...
    if (!this.row.splitBelow) {
      drawLine.call(table.page, this.options.borderWidth,
                    [x - halfBorderWidth, table.y - height + halfBorderWidth],
                    [x + columns[column], table.y - height + halfBorderWidth])
    }
    // border top
    if (!this.row.splitAbove) {
      drawLine.call(table.page, this.options.borderWidth,
                    [x - halfBorderWidth, table.y],
                    [x + columns[column], table.y])
    }
    ...
    // border right
    drawLine.call(table.page, this.options.borderWidth,
                  [x + columns[column] - halfBorderWidth, downTo],
                  [x + columns[column] - halfBorderWidth, table.y - halfBorderWidth])
    // border left
    drawLine.call(table.page, this.options.borderWidth,
