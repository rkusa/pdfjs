  ...
  async _render(text, opts) {
    ...
    while (nextWord !== null || postponeLinebreak || (bk = breaker.nextBreak()) || (isLastTextChunk && this._line.length > 0)) {
      ...
      // i.e., skip to the line rendering
        ...
        // remove surrounding whitespaces if white-space style is set to normal
      ...
      const notEnoughSpace = this._spaceLeft < wordWidth
      if (postponeLinebreak || (bk && bk.required) || isLastWord || notEnoughSpace) {
        ...
        if (this._line.length === 0 && notEnoughSpace) {
          ...
          for (let i = word.length - 1; i >= 0; --i) {
            wordWidth -= font.stringWidth(word[i], fontSize)

            if (this._spaceLeft >= wordWidth) {
              this._line.push({ l: wordWidth, w: word.slice(0, i), f: font, s: fontSize, c: color })
