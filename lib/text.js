  ...
  async _render(text, opts) {
    ...
    while (nextWord !== null || postponeLinebreak || (bk = breaker.nextBreak()) || (isLastTextChunk && this._line.length > 0)) {
      ...
      if (postponeLinebreak || (bk && bk.required) || isLastWord || notEnoughSpace) {
            ...
            if (this._spaceLeft >= offsetWidth) {
              this._line.push(
                new TextChunk({
                  ...
                  font, fontSize, color, decoration, link
                })
              )
        ...
        const underlineStyle = new UnderlineRangeStyle(this._doc, left, this._cursor.y, height, spacing)
        const strikethroughStyle = new StrikethroughRangeStyle(this._doc, left, this._cursor.y, height, spacing)
        const linkStyle = new LinkRangeStyle(this._doc, left, this._cursor.y, height, spacing)
        ...
        if (goTo) {
          ...
          const annot = new PDF.Dictionary({
            ...
            A: new PDF.Dictionary({
              // Type: 'Action',
              S: 'GoTo',
              D: new PDF.String(goTo),
            }),
          })
          this._doc._annotations.push(annot)
        }
        if (destination) {
          this._doc._destinations.set(destination, new PDF.Array([
            this._doc._currentPage.toReference(),
            new PDF.Name('XYZ'),
            left,
            this._cursor.y,
            null,
