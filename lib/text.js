...
import {PDFDictionary, PDFArray, PDFName, PDFString} from './object'

const UNDERLINE_FLAG = 1
const STRIKETHROUGH_FLAG = 2

// TODO: do not extend from Fragment
export default class Text extends Fragment {
  constructor(doc, parent, opts) {
    ...
    this.defaultColor = opts.color && colorToRgb(opts.color) || this._doc.defaultColor
  ...
  async _render(text, opts) {
    ...
    const color = opts.color && colorToRgb(opts.color) || this.defaultColor
    ...
    while (nextWord !== null || postponeLinebreak || (bk = breaker.nextBreak()) || (isLastTextChunk && this._line.length > 0)) {
      ...
      if (postponeLinebreak || (bk && bk.required) || isLastWord || notEnoughSpace) {
        ...
        for (let i = 0; i < this._line.length; ++i) {
          ...
          const colorChanged = !rgbEqual(w.color, this._previousColor)
...
class LinkRangeStyle extends RangeStyle {
  ...
  _applyStyle(textChunk) {
    this.doc._annotations.push(new Dictionary({
      Type: 'Annot',
      Subtype: 'Link',
      Rect: new Array([this.from, this.y, this.from + this.width, this.y + this.height]),
      Border: new Array([0, 0, 0]),
      A: new Dictionary({
        Type: 'Action',
        S: 'URI',
        URI: new String(this.link),
      }),
    }))
    return ''
  }
...
class DestinationRangeStyle extends RangeStyle {
  ...
  _applyStyle(textChunk) {
    this.doc._destinations.set(this.destination, new Array([
