      ...
      if (postponeLinebreak || (bk && bk.required) || isLastWord || notEnoughSpace) {
        ...
        for (const i in this._line) {
          ...
          if (spacing === 0) {
            ...
            out.push(w.w + (!isLast && w.l[1] > 0 ? ' ' : ''))
          } else {
            out.push(font.encode(w.w), calcSpaceWidth(spacing, w.f, w.s))
          }

          let shift = w.l[0] + ((i > 0 && w.l[1] > 0) ? (spacing || w.l[1]) : 0)
  ...
  append(text, opts) {
    ...
    this._pending.push(() => {
      if (this._line.length > 0) {
        const word = this._line[this._line.length - 1]
        this._spaceLeft += word.l[1]
        word.l[1] = 0 // set space width to zero
      }
