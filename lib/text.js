  ...
  async _render(text, opts) {
    ...
    while (nextWord !== null || postponeLinebreak || (bk = breaker.nextBreak()) || (isLastTextChunk && this._line.length > 0)) {
      ...
      if (postponeLinebreak || (bk && bk.required) || isLastWord || notEnoughSpace) {
        ...
        const underlineStyle = new RangeStyle(left, this._cursor.y, spacing)
        ...
        for (let i = 0; i < this._line.length; ++i) {
          ...
          if ((!(w.decoration & STRIKETHROUGH_FLAG) || fontStyleChanged || colorChanged) && shouldStrikethrough) {
            chunk += drawStrikethrough(this._previousFontSize)
            shouldStrikethrough = false
          }

          if ((w.decoration & STRIKETHROUGH_FLAG) && !shouldStrikethrough) {
            shouldStrikethrough = true
            strikethroughFrom += strikethroughWidth
            strikethroughWidth = 0
            if (i > 0) {
              strikethroughFrom += font.stringWidth(' ', this._previousFontSize)
              strikethroughWidth = -font.stringWidth(' ', w.fontSize)
            }
          }
          ...
          if (spacing === 0) {
            ...
            out.push(font.encode(w.word), calcSpaceWidth(spacing, w.font, w.fontSize))
          }

          let shift = w.wordWidth + ((i > 0 && w.spaceWidth > 0) ? (spacing || w.spaceWidth) : 0)
...
class RangeStyle {
  ...
  applyStyle(textChunk, isLast, fontStyleChanged) {
    const shouldUnderline = textChunk.decoration & UNDERLINE_FLAG
    let chunk = ''

    if (shouldUnderline && !fontStyleChanged && this.isActive) {
      this.width += this.lastSpaceWidth
    }

    if (this.isActive && (!shouldUnderline || fontStyleChanged)) {
      chunk += this._applyStyle(textChunk)
      this.from += this.width + this.lastSpaceWidth
      this.width = 0
    }

    if ((!this.isActive || shouldUnderline) || this.isActive && fontStyleChanged) {
  ...
  _applyStyle(textChunk) {
    ...
    const chunk = ops.w(this.underlineThickness)  // line width
                + ops.SC(...textChunk.color) // stroking color
