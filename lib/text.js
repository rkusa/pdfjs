    ...
    while ((bk = breaker.nextBreak()) || (isLastTextChunk && this._line.length > 0)) {
      ...
      if ((bk && bk.required) || isLastWord || linebreaks > 0 || (this._spaceLeft - wordWidth) < 0) {
        ...
        if (word && (this._spaceLeft - wordWidth) >= 0) {
          this._line.push({ l: wordWidth, w: word })
        ...
        // const freeSpace = this._cursor.width - this._line.map(w => w.l).reduce((l, r) => l + r, 0)
        ...
        if (this._previousFont !== font) {
          this._doc._useFont(font)
          chunk += ops.Tf(font.alias, fontSize)
        }

        chunk += ops.Tm(1, 0, 0, 1, left, this._cursor.y, this._cursor.x, height) // set pos
        if (spacing === 0) {
          chunk += ops.Tj(font.encode(this._line.map(w => w.w).join(' ')))
        } else {
          const arr = []
          // TODO use word specific fontSize
          const spaceWidth = -(spacing + font.stringWidth(' ', fontSize)) * scaleFactor
          this._line.forEach(w => {
            arr.push(
              font.encode(w.w),
              spaceWidth
            )
