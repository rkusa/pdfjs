...
module.exports = class Cursor {
  ...
  fork() {
    const fork = new this.constructor(this.width, this.height, this.startX, this.startY)
    fork.x = this.x
    fork.y = this.y
    fork.bottom = this.bottom
    fork._beforeBreakHandler = this._beforeBreakHandler.slice()
    fork._afterBreakHandler = this._afterBreakHandler.slice()
    for (const key in this._stack) {
      fork._stack[key] = this._stack[key].slice()
    }
    return fork
  }
