...
module.exports = class ExternalDocument {
  ...
  async write(doc, page) {
    return await this._writer(doc, page ? (i) => (i === (page - 1)) : undefined)
  }

  async writePageRange(doc, start, end) {
    return await this._writer(doc, (i) => ((start ? (start <= i) : true) && (end ? (i <= end) : true)), {i: start ? start : 0})
  }

  async writePages(doc, pages) {
    return await this._writer(doc, (i) => pages.includes(i + 1))
  }

  // TODO: add mutex to not write concurrently (because of document specific _registerObject)
  async _writer(doc, filter, cursor) {
    await doc._endPage()

    const kids = this.pages.get('Kids')
    
    for (const page of this._iterPagesRecursively(doc, kids, filter, cursor)) {
