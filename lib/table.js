...
module.exports = class Table {
  ...
  async _renderHeader(isPageBreak) {
    ...
    if (isPageBreak && this._header.offset > 0) {
      // offset header to the top
      chunk += ops.q()
             + ops.cm(1, 0, 0, 1, 0, this._header.offset)
    }
    ...
    if (isPageBreak && this._header.offset > 0) {
      chunk +=  ops.Q()
      this._cursor.y += this._header.offset
    }
  ...
  row(opts) {
    return this._row(opts, require('./row'))
  }

  header(opts) {
    const ctx = this._row(opts, require('./tableheader'))
    this._header = ctx
