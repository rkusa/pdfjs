...
CursorFactory.prototype.pageBreak = function(pageBreak, parent) {
  if (!pageBreak) {
    ...
    if (this.currentPage in this.pageBreaks && this.visitedBreaks && this.visitedBreaks[this.pageBreaks[this.currentPage].id] === true) {
      console.log('TAKE')
    ...
    if (!this.validBreaks[pageBreak.id] || (this.visitedBreaks && !this.visitedBreaks[pageBreak.id] && !this.visitedBreaks[pageBreak.id - 1])) {
      this.validBreaks = this.visitedBreaks
      delete this.pageBreaks[this.currentPage]
      return null
    }

    if (!this.visitedBreaks) {
