...
class Document extends Readable {
  ...
  async _startPage() {
    const page = this._currentPage = new PDF.Page(this._pages)
    ...
    if (this._footer) {
      ...
      for (const i in this._footer._objects) {
        ...
        const alias = new PDF.Name('Ftr' + i)
        this._currentContent._xobjects[alias] = obj.toReference()
        chunk += ops.Do(alias)
      }

      await this._write(chunk)
      await this._footer.writePageNumbers()
    }
    ...
    if (this._footer) {
      this._cursor.bottom += this._footer.height
    }
  }
  ...
  async _endPage() {
    ...
    await this._currentPage.write(this)
  ...
  async _startContentObject(obj) {
    ...
    if (!obj && '_createObject' in this._currentPage) {
      obj = this._currentPage._createObject()
    }
