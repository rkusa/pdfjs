...
class Document extends Readable {
  ...
  addPageRangeOf(external, start, end) {
    if (!(external instanceof ExternalDocument)) {
      throw new TypeError('argument must be of type ExternalDocument')
    }

    if (!start || start < 1 || start > external.pageCount) {
      throw new TypeError('ExternalDocument does not have page ' + start)
    }

    if (!end || end < 1 || end > external.pageCount) {
      throw new TypeError('ExternalDocument does not have page ' + end)
    }

    this._begin(null)
    this._pending.push(() => external.writePageRange(this, start, end))
  }

  addSpecificPagesOf(external, pages) {
    if (!(external instanceof ExternalDocument)) {
      throw new TypeError('argument must be of type ExternalDocument')
    }

    const invalidPages = pages.filter(i => ((i < 1) || (i > external.pageCount)))

    if (invalidPages.length > 0) {
      throw new TypeError(`External Document does not have pages ` + invalidPages)
    }

    this._begin(null)
    this._pending.push(() => external.writePages(this, pages))
  }
