...
class Document extends Readable {
  constructor(opts) {
    ...
    this._rootFragment = new Fragment(this, this);
  }
  ...
  asBuffer(callback) {
    let p = new Promise((resolve, reject) => {
      const chunks = []
      this._rootFragment.on('data', chunk => chunks.push(chunk))
      this._rootFragment.on('end', () => resolve(Buffer.concat(chunks)))
      this._rootFragment.on('error', reject)
      this._rootFragment.end()
    })
  ...
  header() {
    ...
    const ctx = new Header(this, this._rootFragment)
    this._rootFragment._begin(ctx)
  ...
  footer() {
    ...
    const ctx = new Footer(this, this._rootFragment)
    this._rootFragment._begin(ctx)
  ...
  addPagesOf(external) {
    ...
    this._rootFragment._begin(null)
  ...
  text(text, opts) {
    return this._rootFragment.text(text, opts)
  }

  cell(text, opts) {
    return this._rootFragment.cell(text, opts)
  }

  table(opts) {
    return this._rootFragment.table(opts)
  }

  image(img, opts) {
    this._rootFragment.image(img, opts)
  }

  pageBreak() {
    this._rootFragment.pageBreak()
  }

  op(fn) {
    Fragment.prototype.op.apply(this._rootFragment, arguments);
  }

  destination(name) {
    this._rootFragment.destination(name);
  }
}
