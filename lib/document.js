...
module.exports = class Document extends Readable {
  ...
  startPage() {
    ...
    this.xref.add(page.contents.id, this._readableState.length, page.contents)

    this.write(page.contents.id + ' ' + page.contents.rev + ' obj\n')
    this.write(page.contents.properties.toString() + '\n')
    this.write('stream\n')

    this.pageStart = this._readableState.length
  }

  endPage() {
    const length = this._readableState.length - this.pageStart - 1
