...
class DocumentBase extends Readable {
  ...
  async _endPage() {
    ...
    // reset cursor first, to trigger before break handlers
...
module.exports = class Document extends DocumentBase {
  ...
  async _cell(text, opts) {
    ...
    let bgLayerIndex = this._pageContents.content.length - 1
    ...
    const createBackground = async () => {
      ...
      // write background
      const height = previousCursor.y - this._cursor.y + padding
      const chunk = ops.q()
        + ops.sc(backgroundColor[0], backgroundColor[1], backgroundColor[2])
        + ops.re(previousCursor.x, previousCursor.y - height, outerWidth, height)
        + ops.f()
        + ops.Q()
      await this._write(chunk)

      // TODO: calculate new bgLayerIndex from hierarchy stack
      bgLayerIndex = 0
    }
