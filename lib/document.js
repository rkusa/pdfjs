...
Document.prototype.startTransaction = function() {
  this.transaction = { page: this.pages.count - 1, length: this.cursor.contents.content.length }
}

Document.prototype.rollback = function() {
  if (!this.transaction) return
  
  if (this.transaction.page < (this.pages.count - 1)) {
    for (var i = this.pages.count - 1; i > this.transaction.page; --i)
      this.pages.removePageAt(i)
    this.cursor = this.pages.pages[this.transaction.page]
  }

  if (this.transaction.length < this.cursor.contents.content.length) {
    this.cursor.contents.content = this.cursor.contents.content.slice(0, this.transaction.length)
  }
}
...
Document.prototype.pagebreak = function() {
  return this.cursor = this.pages.addPage()
}
