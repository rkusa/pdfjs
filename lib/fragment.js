...
module.exports = class Fragment {
  ...
  text(text) {
    if (text) {
      this._doc._pending = this._doc._pending.then(() => this._text(text, this._cursor))
    }
  }

  async _text(text, cursor) {
    ...
    while ((bk = breaker.nextBreak()) || line.length > 0) {
      ...
      if ((bk && bk.required) || isLastWord || linebreaks > 0 || (spaceLeft - wordWidth) < 0) {
        ...
        case 'right':
          ...
          break
        case 'center':
          left += cursor.width / 2 - (cursor.width - freeSpace) / 2
          break
        case 'justify':
          const isLastLine = isLastWord || linebreaks > 0
          if (isLastLine && freeSpace / cursor.width > .2) {
            break
          }
        ...
        chunk += ops.Tm(1, 0, 0, 1, left, cursor.y, cursor.x, height) // set pos
        if (spacing === 0) {
        ...
        cursor.y += descent // add, because descent is negative
      ...
      if (linebreaks > 0) {
        cursor.y -= font.lineHeight(fontSize, true) * lineHeight * linebreaks
      }
    }
