  ...
  if (Array.isArray(this.opts.width)) {
    ...
    var highestRowWidth = Math.max.apply(Math, this.rows.map(function(row) {
      ...
      return row.cells.map(function(cell) {
        if (columns[colIndex] === undefined || (cell.width > columns[colIndex] && !(cell.opts.colspan > 1))) {
          columns[colIndex] = cell.width
        }

        if (cell.opts.colspan > 1) {
          for (var i = 1; i < cell.opts.colspan; ++i) {
            colIndex++
            columns[colIndex] = columns[colIndex] || 0
          }
        }
...
Row.prototype.render = function(page, columns, context) {
  ...
  var index = page.contents.content.length

  function addBorder(x, y, width) {
    borders.push(function(height) {
      cell.drawBorder(page, x, y, width, height, index)
    })
  }
...
Cell.prototype.drawBorder = function(page, x, y, width, height, splitBelow, splitAbove, index) {
  if (y - this.doc.padding.bottom < height) {
    var spaceLeft = height - y + this.doc.padding.bottom
    var pageIndex = this.doc.pages.pages.indexOf(page)
    this.drawBorder(page, x, y, width, y - this.doc.padding.bottom, true, false, index)
    while (spaceLeft > 0) {
      page = this.doc.pages.pages[++pageIndex]
      if (spaceLeft > this.doc.innerHeight) {
        height = this.doc.innerHeight
        this.drawBorder(page, this.doc.padding.left, this.doc.height - this.doc.padding.top, width, height, true, true, 0)
      } else {
        height = spaceLeft // this.doc.height - this.doc.padding.top - page.cursor.y
        this.drawBorder(page, this.doc.padding.left, this.doc.height - this.doc.padding.top, width, height, false, true, 0)
      }
      spaceLeft -= height
    }
    return
  ...
  if (this.opts.backgroundColor) {
    drawBackground(page, x, y - height, width, height, this.opts.backgroundColor, index)
  }
}
...
function drawLine(page, width, from, to, color) {
  page.contents.writeLine(utils.colorToRgb(color || 0x000000).join(' ') + ' RG')
...
function drawBackground(page, x, y, width, height, color, index) {
  page.contents.content.splice.apply(page.contents.content, [
    index, 0,
