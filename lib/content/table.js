...
Table.prototype.prepare = function(width) {
  ...
  this.rows.forEach(function(row) {
    ...
    row.cells.forEach(function(cell) {
      ...
      if (cell.opts.colspan > 1) {
        for (var i = column + 1, len = column + cell.opts.colspan; i < len; ++i) {
          width += columns[i] || 0
        }
      ...
      cell.lines = cell.text.prepare(cell.innerWidth)
      cell.innerHeight = cell.lines.height
...
Object.defineProperties(Row.prototype, {
  ...
  height: {
    enumerable: true,
    get: function() {
      return Math.max.apply(Math, this.cells.map(function(cell) {
        return cell.height
      }))
    }
  }
...
Row.prototype.render = function(page, y, x) {
  var self = this, height = this.height  
  
  // if (pos > 0) this.y += this.opts.borderWidth.top
  
  this.cells.forEach(function(cell, column) {
    cell.render(page, y, x, height, column)
    x += cell.width 
  })
}

Row.prototype.break = function(spaceLeft) {
  var self = this, height = this.height
  
  // consider header row, if required
  if (this.opts.header === true && !this.splitAbove) {
    height += this.table.rows[0].height
  }
  ...
  // the row has do be splitted to fit into one page
    ...
    // create new row
    var row = new Row(this.table, this.opts, function(row) {
      // split each cell
      self.cells.forEach(function(cell) {
        var newCell = new Cell(row, cell.text, cell.opts)
        var succ = cell.lines.break(spaceLeft)
        newCell.lines = succ
        newCell.innerWidth = cell.innerWidth
        newCell.innerHeight = newCell.lines.height
        row.cells.push(newCell)
      })
      row.splitAbove = self.splitBelow = true
    })
...
var Cell = function(row, text, opts) {
  ...
  if (text instanceof Text) {
    ...
    this.text = new Text(this.doc, this.opts)
    if (typeof text === 'function')
      text.call(this.text, this.text.textFn) 
    else
      this.text.text(text)
  }

  this.innerWidth  = this.text.maxWidth
  this.innerHeight = this.text.maxLineHeight
...
Cell.prototype.render = function(page, y, x, height, column) {
  ...
  var downTo = y - height
  if (downTo < page.padding.bottom)
    downTo = page.padding.bottom
    
  // border right
  if (this.borderRightWidth > 0) {
    drawLine(page, border = this.borderRightWidth,
             [x + this.width - border / 2, downTo],
             [x + this.width - border / 2, y - border / 2])
  }
  ...
  this.lines.render(
    page,
    y - this.opts.padding.top - this.borderTopWidth,
    x + this.opts.padding.left + this.borderLeftWidth
  )
}
