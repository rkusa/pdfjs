...
Row.prototype.render = function(page, columns) {
  ...
  for (var column = 0, len = this.cells.length; column < len; ++column) {
    ...
    var height = Math.min(y - page.cursor.y + cell.opts.padding.bottom + cell.borderBottomWidth, this.doc.innerHeight)
    for (var i = pageIndex + 1, count = this.doc.pages.count; i < count; ++i) {
      height += this.doc.height - this.doc.padding.top - this.doc.pages.pages[i].cursor.y
    }
    heights.push(height)
    page.cursor.y = y
