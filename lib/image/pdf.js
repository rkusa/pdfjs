...
const ImageSet = require('./set')
const PDF = require('../object')
const Parser = require('../parser/parser')

module.exports = class PDFImage {
  constructor(src, set) {
    this.alias = new PDF.Name((set || ImageSet.global).nextId())
  ...
  async write(doc, xobj) {
    ...
    const objects = []
    addObjectsRecursive(objects, xobj)

    // first, register objects to assign IDs (for references)
    for (const obj of objects) {
      await doc._registerObject(obj)
    }
...
function addObjectsRecursive(objects, value) {
  switch (true) {
    case value instanceof PDF.Reference:
      ...
      addObjectsRecursive(objects, value.object)
      break
    case value instanceof PDF.Object:
      addObjectsRecursive(objects, value.properties)
      addObjectsRecursive(objects, value.content)
      break
    case value instanceof PDF.Dictionary:
      for (var key in value.dictionary) {
        addObjectsRecursive(objects, value.dictionary[key])
      }
      break
    case Array.isArray(value):
      value.forEach(function(item) {
        addObjectsRecursive(objects, item)
      })
      break
  }
}
